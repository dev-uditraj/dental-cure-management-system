...
public class EditBillD extends JFrame {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					EditBillD frame = new EditBillD(img);
...
public static void updatetable()
	
	{
		...
		else if(d==true)
		{
				...
				int count=0;
				try {
				rs=st.executeQuery("select billid,fk_appointmentid,datetime from billtable where fk_patientid="+Integer.parseInt(SearchInfoDentistt.no.toString())+" order by datetime asc");
				while(rs.next())
				{
					count++;
					int billid= rs.getInt(1);
	...
	public EditBillD(Image img) {
		...
		btnView.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				{
					ViewBillD.getbillid=table.getValueAt(row,1).toString();
					ViewBillD.main(null);
				}
			}
		});
