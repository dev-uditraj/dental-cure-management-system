...
public class AppointmentMList extends JFrame {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					frame = new AppointmentMList(img);
	 ...
	 * Create the frame.
	 */
	public AppointmentMList(String img) {
	...
	public AppointmentMList(Image img) {
      setMinimumSize(new Dimension(600, 401));
		...
		dateChooser.setBounds(222, 80, 188, 31);
		...
		btn_attach.setBounds(109, 301, 186, 31);
		btn_attach.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				int column = 1;
				int row = EditAppointment.table.getSelectedRow();				
				String value = EditAppointment.table.getModel().getValueAt(row, column).toString();
				long getappointmentid = Integer.parseInt(value);
					
				try {
					con.createStatement().execute("update appointmenttable set datetime='"+finaldateandtime+"' where appointmentid="+getappointmentid+"");
				}
				catch(Exception e)
				{
				e.printStackTrace();	
				...
				EditAppointment.model.setRowCount(0);
				EditAppointment.updatetable();	
		...
		btn_clear.setBounds(313, 301, 183, 31);
