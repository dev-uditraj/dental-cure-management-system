...
public class Appointment extends JFrame {
	...
	public Appointment(Image img) {
         setMinimumSize(new Dimension(600, 401));
		...
		dateChooser.setBounds(208, 81, 188, 31);
		...
		label_1.setBounds(66, 150, 97, 22);
		...
		btn_attach.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(a==true)
				{
					...
					Patientreg.transferdate= todaydate;
				}
				else if(b==true)
				{
					try {
						...
						Patientreg.transferdate=nextDate;
				    }
					catch(Exception e){	}
				...
				else if(c==true)
				{
					...
					Date date2 = new SimpleDateFormat("yyyy-MM-dd").parse(todaydate);
							...
							if(isdatevalid<0)
							{
								JOptionPane.showMessageDialog(null,"the date selected is before the current date, please select other date!");
							exception=1;
							}
							else if(isdatevalid==0)
							{
								JOptionPane.showMessageDialog(null,"The date selected is the current date, please select the today's date radio button instead!");
							exception=1;
							}
							else
							{
								 exception =0;
								 Patientreg.transferdate=custom;
							}
					}
					catch(Exception e)
					{
				...
				Patientreg.transfertime=wholetime;
				Patientreg.transferappointmentid= textField.getText();
				if(st!=null)
				{
					st=null;
				}
				...
				Patientreg.textArea.setText("Appointment ID= "+Patientreg.transferappointmentid+"\nDate= "+Patientreg.transferdate+"\nTime= "+Patientreg.transfertime+"");
				frame.dispose();
		...
		comboBox.setBounds(208, 150, 83, 31);
		...
		comboBox_1.setBounds(303, 150, 83, 31);
		...
		comboBox_2.setBounds(398, 150, 79, 31);
		...
		textField.setBounds(208, 206, 116, 31);
